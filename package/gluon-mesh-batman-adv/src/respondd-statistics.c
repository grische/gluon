...
/* Batman IV: gateway with TQ metric */
...
static int parse_gw_list_iv_netlink_cb(struct nl_msg *msg, void *arg)
{
	struct nlattr *attrs[BATADV_ATTR_MAX+1];
	struct nlmsghdr *nlh = nlmsg_hdr(msg);
	struct batadv_nlquery_opts *query_opts = arg;
...
/* Batman V: gateway with throughput metric */
...
static int parse_gw_list_v_netlink_cb(struct nl_msg *msg, void *arg)
{
	struct nlattr *attrs[BATADV_ATTR_MAX+1];
	struct nlmsghdr *nlh = nlmsg_hdr(msg);
	struct batadv_nlquery_opts *query_opts = arg;
...
static void add_gateway(struct json_object *obj) {
	...
	if (strcmp(algoname, "BATMAN_IV") == 0) {
		batadv_genl_query("bat0", BATADV_CMD_GET_GATEWAYS,
				parse_gw_list_iv_netlink_cb, NLM_F_DUMP,
				&opts.query_opts);
	} else if (strcmp(algoname, "BATMAN_V") == 0) {
		batadv_genl_query("bat0", BATADV_CMD_GET_GATEWAYS,
				parse_gw_list_v_netlink_cb, NLM_F_DUMP,
				&opts.query_opts);
	}
}
